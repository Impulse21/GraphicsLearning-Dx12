...
Dx12Core::CpuDescriptorHeap::CpuDescriptorHeap(
	GraphicsDevice& graphicsDevice,
	uint32_t numDesctiptors,
	D3D12_DESCRIPTOR_HEAP_TYPE type,
	D3D12_DESCRIPTOR_HEAP_FLAGS flags)
	: m_graphicsDevice(graphicsDevice)
	, m_descriptorSize(graphicsDevice.GetDx12Device2()->GetDescriptorHandleIncrementSize(type))
...
DescriptorHeapAllocation Dx12Core::CpuDescriptorHeap::Allocate(uint32_t numDescriptors)
{
...
void Dx12Core::CpuDescriptorHeap::Free(DescriptorHeapAllocation&& allocation)
{
	struct StaleDescriptorAllocation
	{
		DescriptorHeapAllocation Allocation;
		CpuDescriptorHeap* Heap;

		StaleDescriptorAllocation(DescriptorHeapAllocation&& allocation, CpuDescriptorHeap* heap)
			: Allocation(std::move(allocation))
			, Heap(heap)
		{}

		StaleDescriptorAllocation(const StaleDescriptorAllocation&) = delete;
		StaleDescriptorAllocation& operator= (const StaleDescriptorAllocation&) = delete;
		StaleDescriptorAllocation& operator= (StaleDescriptorAllocation&&) = delete;

		StaleDescriptorAllocation(StaleDescriptorAllocation&& other) noexcept
	...
	StaleDescriptorAllocation staleAllocation(std::move(allocation), this);
	this->m_graphicsDevice.SafeReleaseObject(std::move(staleAllocation));
}

std::shared_ptr<DescriptorHeapAllocationPage> Dx12Core::CpuDescriptorHeap::CreateAllocationPage()
{
	auto newPage = std::make_shared<DescriptorHeapAllocationPage>(
		this->m_heapPool.size(),
		this,
		this->m_graphicsDevice.GetDx12Device2(),
		this->m_heapDesc,
...
void Dx12Core::CpuDescriptorHeap::FreeAllocation(DescriptorHeapAllocation&& allocation)
{
...
Dx12Core::DescriptorHeapAllocationPage::DescriptorHeapAllocationPage(
	uint32_t id,
...
DescriptorHeapAllocation Dx12Core::DescriptorHeapAllocationPage::Allocate(uint32_t numDescriptors)
{
...
void Dx12Core::DescriptorHeapAllocationPage::Free(DescriptorHeapAllocation&& allocation)
{
	std::scoped_lock _(this->m_allocationMutex);
...
uint32_t Dx12Core::DescriptorHeapAllocationPage::ComputeCpuOffset(D3D12_CPU_DESCRIPTOR_HANDLE handle)
{
	return static_cast<uint32_t>(handle.ptr - this->m_baseCpuDescritpor.ptr) / this->m_descritporSize;
}

void Dx12Core::DescriptorHeapAllocationPage::AddNewBlock(uint32_t offset, uint32_t numDescriptors)
{
...
void Dx12Core::DescriptorHeapAllocationPage::FreeBlock(uint32_t offset, uint32_t numDescriptors)
{
...
Dx12Core::GpuDescriptorHeap::GpuDescriptorHeap(
	GraphicsDevice& graphicsDevice,
	uint32_t numDesctiptors,
	...
	, m_descriptorSize(graphicsDevice.GetDx12Device2()->GetDescriptorHandleIncrementSize(type))
{
	...
	ThrowIfFailed(
		this->m_graphicsDevice.GetDx12Device2()->CreateDescriptorHeap(
			&this->m_heapDesc,
...
void GpuDescriptorHeap::Free(DescriptorHeapAllocation&& allocation)
{
	struct StaleDescriptorAllocation
	{
		...
		StaleDescriptorAllocation(DescriptorHeapAllocation&& allocation, GpuDescriptorHeap* heap)
			: Allocation(std::move(allocation))
			, Heap(heap)
		{}
...
void Dx12Core::GpuDescriptorHeap::FreeAllocation(DescriptorHeapAllocation&& allocation)
{
...
void Dx12Core::DynamicSuballocator::ReleaseAllocations()
{
...
DescriptorHeapAllocation Dx12Core::DynamicSuballocator::Allocate(uint32_t numDescriptors)
{
...
void Dx12Core::DynamicSuballocator::Free(DescriptorHeapAllocation&& allocation)
{
	allocation.Reset();
}

Dx12Core::DynamicSuballocator::DynamicSuballocator(
	GpuDescriptorHeap& gpuDescriptorHeap,
